...
UI나 디자인 작업에서는 시각 스타일을 잡기 전에 Substrate Decision Gate를 먼저 거칩니다. 즉 DOM, CSS layout, canvas, SVG, WebGL, fixed-stage, hybrid 중 어떤 구현 매체가 제품 도메인에 맞는지 먼저 고릅니다. 이 흐름은 픽셀 에디터, 드로잉 도구, 그래프 편집기, 슬라이드 덱처럼 일반적인 DOM 카드/그리드로 만들면 어색하거나 무거워지는 작업을 피하도록 돕습니다. 예를 들어 픽셀 에디터라면 canvas 작업면과 DOM 툴바, 팔레트, inspector를 조합하는 식으로 생각합니다.
...
- 시각 스타일이나 코드 작성 전에 product archetype과 구현 substrate를 먼저 고릅니다.
- canvas, SVG, WebGL, fixed-stage, hybrid 구조가 필요한 작업을 무리하게 DOM 카드/그리드로 만들지 않도록 점검합니다.
