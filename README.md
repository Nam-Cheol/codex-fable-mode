...
v0.2.0부터는 특정 주제나 시각 스타일이 아니라 작업 행위 자체를 더 강하게 감시합니다. 어린이/우주/디자인 전용 모드가 아닙니다. 핵심 제약을 지켰는지, 출력 형식을 잘못 해석하지 않았는지, 필요한 근거와 도구를 확인했는지, 불충분한 결과를 그럴듯하게 포장하지 않았는지, 작업 깊이가 적절한지, 최종 결과가 사용자의 의도에 맞는지 점검합니다.
