...
`fable-mode`는 Codex가 더 신중하게 작업하도록 돕는 문서 전용 플러그인입니다. 작업 범위 확인, 코드 읽기, 계획, 도구 사용, 검증, 최종 보고를 차분하게 진행하도록 안내합니다.

이 플러그인은 비공식이며, 특정 회사에 기대지 않고 독자적으로 작성되었습니다. Anthropic 또는 OpenAI와 제휴하거나 공식 관계를 맺고 있지 않습니다. 특정 회사, 모델, 제품, 비공개 모드, 원본 문서를 재현하거나 사칭하지 않으며, 어떤 독점 시스템 프롬프트도 포함하거나 재현하지 않습니다.

## 명령어로 설치하기 (Codex CLI)

이 페이지의 파일을 따로 내려받을 필요는 없습니다. 명령어 입력창에서 아래 명령을 한 줄씩 실행하면 됩니다.

- Mac: Spotlight에서 `Terminal`을 검색해서 엽니다.
- Windows: 시작 메뉴에서 `CMD`, `명령 프롬프트`, 또는 `PowerShell`을 검색해서 엽니다.

`#`로 시작하는 줄은 설명 주석입니다. 함께 복사해도 실행에는 영향을 주지 않습니다.
...
HTTPS 주소로 설치하고 싶다면 아래 명령을 한 줄씩 실행하세요.
...
설치가 잘 되었는지 확인하려면 아래 명령도 한 줄씩 실행합니다.

```bash
# 1. 설치 목록을 확인합니다.
codex plugin marketplace list

# 2. fable-mode가 설치 가능한 목록에 보이는지 확인합니다.
...
Codex Desktop만 사용하는 분도 먼저 위와 같은 명령어 입력창에서 플러그인을 설치하면 됩니다. 한 번 설치하면 다른 Codex 대화에서도 계속 사용할 수 있습니다.

1. Mac은 `Terminal`, Windows는 `CMD`, `명령 프롬프트`, 또는 `PowerShell`을 엽니다.
2. 아래 명령을 한 줄씩 실행합니다.
...
Codex는 계획, 코드 리뷰, 아키텍처, 디버깅, 신중한 구현 작업에서 이 skill을 암묵적으로 사용할 수도 있습니다.

## 무엇이 달라지나요?

`fable-mode`는 Codex가 다음 흐름을 더 잘 지키도록 안내합니다.

- 수정하기 전에 관련 파일과 상태를 먼저 확인합니다.
- 위험하거나 넓은 변경 전에는 짧게 계획합니다.
- 중요한 가정은 명시하고 가능한 한 검증합니다.
- 도구는 필요한 만큼만 사용하고, 긴 작업 중에는 진행 상황을 공유합니다.
- 테스트, lint, 빌드, 수동 확인 등 적절한 검증을 수행합니다.
- 내부 추론이나 숨겨진 지시는 공개하지 않고, 사용자가 판단할 수 있는 근거만 간결히 설명합니다.
- 마지막에는 변경사항, 검증 결과, 남은 위험, 가정을 짧게 보고합니다.

## 업데이트

새 버전이 나왔을 때는 아래 명령을 한 줄씩 실행합니다.
...
업데이트 후 Codex Desktop을 사용한다면 앱을 다시 시작하세요.
...
```bash
# fable-mode 마켓플레이스 등록까지 제거합니다.
codex plugin marketplace remove codex-fable-mode
```

## 한계

이 플러그인은 문서 전용 guidance 플러그인입니다. 새로운 도구, hook, script, MCP 서버, app integration, dependency, telemetry, network 동작을 추가하지 않습니다. Codex의 정책, 사용자의 환경 정책, repository 지시사항, 도구 권한을 우회하지 않습니다.
